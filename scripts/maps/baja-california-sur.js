// scripts/maps/baja-california-sur.js
// ==============================
// IMPORTACIONES
// ==============================
import {
...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Baja California Sur");
...
// ✅ Filtro de entidad para marcadores (Baja California Sur)
const ES_CVE_BCS = "03";
const esDeBajaCaliforniaSurRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_BCS
  || /baja\s*california\s*sur/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/baja-california-sur.geojson"),
  d3.csv("../data/rate/baja-california-sur.csv")
]).then(([geoData, tasasRaw]) => {

  // Año dinámico
  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Baja California Sur ${year}`;
  ...
  renderZoomControles("#mapa-entidad", {
    svg, g, zoom, showHome: true, idsPrefix: "bcs", escalaMin: 1, escalaMax: 20, paso: 0.5
  });
  ...
  async function cargarYPintar(tipo) {
    ...
    // ✅ Solo marcadores de BCS
    const rawBCS = raw.filter(esDeBajaCaliforniaSurRaw);
  ...
  generarTablaMunicipios("../data/rate/baja-california-sur.csv");
  habilitarDescargaExcel("baja-california-sur.xlsx");
  ...
  // ==============================
  // DESCARGAS PNG (con/sin etiquetas)
  // ==============================
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Baja California Sur", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-baja-california-sur-sin-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
    }, 100);
  });
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-baja-california-sur-con-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
      etiquetas.style.display = "none";
    }, 100);
  });

}).catch(err => console.error("Error en mapa de Baja California Sur:", err));
