// scripts/maps/aguascalientes.js
// ==============================
// IMPORTACIONES
// ==============================
import {
...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Aguascalientes");
...
// ✅ Filtro de entidad para marcadores (Aguascalientes)
const ES_CVE_AGS = "01";
const esDeAguascalientesRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_AGS
  || /aguascalientes/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
Promise.all([
  ...
  document.title = `SIARHE | Enfermería en Aguascalientes ${year}`;
  ...
  renderZoomControles("#mapa-entidad", {
    svg, g, zoom, showHome: true, idsPrefix: "ags", escalaMin: 1, escalaMax: 20, paso: 0.5
  });
  ...
  async function cargarYPintar(tipo) {
    ...
    // ✅ Solo marcadores de Aguascalientes (Clave_Entidad=01 o Entidad='Aguascalientes')
    const rawAGS = raw.filter(esDeAguascalientesRaw);
  ...
  habilitarDescargaExcel("aguascalientes.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Aguascalientes", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-aguascalientes-sin-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
    }, 100);
  });
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-aguascalientes-con-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
      etiquetas.style.display = "none";
    }, 100);
  });

}).catch(err => console.error("Error en mapa de Aguascalientes:", err));
