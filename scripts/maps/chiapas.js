...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Chiapas");
...
const ES_CVE_CHIAPAS = "07"; // ✅ Clave INEGI de Chiapas
const esDeChiapasRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_CHIAPAS
  || /chiapas/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/chiapas.geojson"),
  d3.csv("../data/rate/chiapas.csv")
]).then(([geoData, tasasRaw]) => {

  // Año dinámico
  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Chiapas ${year}`;
  ...
  const tasas = tasasRaw.map(d => ({
    ...
    // ámbitos (si existen en CSV; si no, quedan NaN y no afectan)
  ...
  renderZoomControles("#mapa-entidad", {
    svg, g, zoom, showHome: true, idsPrefix: "chps", escalaMin: 1, escalaMax: 20, paso: 0.5
  });
  ...
  async function cargarYPintar(tipo) {
    ...
    // ✅ Solo marcadores de Chiapas
    const rawCHPS = raw.filter(esDeChiapasRaw);
  ...
  generarTablaMunicipios("../data/rate/chiapas.csv");
  habilitarDescargaExcel("chiapas.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Chiapas", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-chiapas-sin-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
    }, 100);
  });
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-chiapas-con-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
      etiquetas.style.display = "none";
    }, 100);
  });

}).catch(err => console.error("Error en mapa de Chiapas:", err));
