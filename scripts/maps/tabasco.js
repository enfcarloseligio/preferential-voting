...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Tabasco");
...
// ✅ Filtro de entidad para marcadores (Tabasco)
const ES_CVE_TABASCO = "27";
const esDeTabascoRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_TABASCO
  || /tabasco/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/tabasco.geojson"),
  d3.csv("../data/rate/tabasco.csv")
]).then(([geoData, tasasRaw]) => {

  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Tabasco ${year}`;
  ...
  renderZoomControles("#mapa-entidad", {
    svg, g, zoom, showHome: true, idsPrefix: "tabs", escalaMin: 1, escalaMax: 20, paso: 0.5
  });
  ...
  async function cargarYPintar(tipo) {
    ...
    const rawTAB = raw.filter(esDeTabascoRaw);
  ...
  generarTablaMunicipios("../data/rate/tabasco.csv");
  habilitarDescargaExcel("tabasco.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Tabasco", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-tabasco-sin-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
    }, 100);
  });
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-tabasco-con-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
      etiquetas.style.display = "none";
    }, 100);
  });

}).catch(err => console.error("Error en mapa de Tabasco:", err));
