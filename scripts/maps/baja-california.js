// scripts/maps/baja-california.js
// ==============================
// IMPORTACIONES
// ==============================
import {
...
// CREACIÓN DEL MAPA
// ==============================
const { svg, g } = crearSVGBase("#mapa-entidad", "Mapa de enfermeras – Baja California");
...
// ✅ Filtro de entidad para marcadores (Baja California)
const ES_CVE_BC = "02";
const esDeBajaCaliforniaRaw = (d) =>
  String(d.Clave_Entidad || d.cve_ent || d.CVE_ENT || d.entidad_id)
    .padStart(2, "0") === ES_CVE_BC
  || /baja\s*california/i.test(String(d.Entidad || d.estado || d.entidad || ""));
...
// ==============================
Promise.all([
  d3.json("../data/maps/baja-california.geojson"),
  d3.csv("../data/rate/baja-california.csv")
]).then(([geoData, tasasRaw]) => {

  // Año dinámico
  const year = new Date().getFullYear();
  document.title = `SIARHE | Enfermería en Baja California ${year}`;
  ...
  renderZoomControles("#mapa-entidad", {
    svg, g, zoom, showHome: true, idsPrefix: "bc", escalaMin: 1, escalaMax: 20, paso: 0.5
  });
  ...
  async function cargarYPintar(tipo) {
    ...
    // ✅ Solo marcadores de Baja California
    const rawBC = raw.filter(esDeBajaCaliforniaRaw);
  ...
  generarTablaMunicipios("../data/rate/baja-california.csv");
  habilitarDescargaExcel("baja-california.xlsx");
  ...
  document.getElementById("descargar-sin-etiquetas")?.addEventListener("click", () => {
    const titulo = construirTitulo(currentMetric, { entidad: "Baja California", year });
    const etiquetas = document.getElementById("etiquetas-municipios");
    if (etiquetas) etiquetas.style.display = "none";
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-baja-california-sin-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
    }, 100);
  });
  ...
  document.getElementById("descargar-con-etiquetas")?.addEventListener("click", () => {
    ...
    setTimeout(() => {
      descargarComoPNG("#mapa-entidad svg", "mapa-baja-california-con-nombres.png", MAP_WIDTH, MAP_HEIGHT, { titulo });
      etiquetas.style.display = "none";
    }, 100);
  });

}).catch(err => console.error("Error en mapa de Baja California:", err));
